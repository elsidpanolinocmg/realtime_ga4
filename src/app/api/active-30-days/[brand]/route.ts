...
const DEFAULT_CACHE_DURATION = 30 * 60_000; // 30 min
...
export async function GET(
  ...
  try {
    ...
    if (cached && now - cached.timestamp < intervalms) {
      console.log(
        `[GA4] ACTIVE LAST 30 DAYS (cache) for ${brand}: ${cached.value}`
      );
    ...
    console.log(
      `[GA4] Fetching ACTIVE LAST 30 DAYS for ${brand} using property ${propertyId}`
    );
    ...
    console.log(
      `[GA4] ACTIVE LAST 30 DAYS fetched for ${brand}: ${value}`
    );
    ...
    console.error("[GA4] Active-30-days error:", message);
